/**
 * Alert Handler — SQLite (CJS)
 */

exports.getAlerts = (req, res, db) => {
...
exports.createAlert = (req, res, db, customer) => {
  ...
  try {
    const result = db.prepare(
      'INSERT INTO dc_alerts (customer_id, camera_id, alert_type, description, confidence, image_url, video_url, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(customer.id, camera_id || null, alert_type, description || null, confidence || 0, image_url || null, video_url || null, metadata ? JSON.stringify(metadata) : null);
...
exports.acknowledgeAlert = (req, res, db) => {
  const { id } = req.params;
  const acknowledgedBy = req.body.acknowledged_by || req.user.username;
  const result = db.prepare("UPDATE dc_alerts SET acknowledged_at=datetime('now'), acknowledged_by=?, updated_at=datetime('now') WHERE id=?").run(acknowledgedBy, id);
...
exports.deleteAlert = (req, res, db) => {
  const result = db.prepare('DELETE FROM dc_alerts WHERE id = ?').run(req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Alert not found' });
  return res.json({ message: 'Alert deleted' });
};

exports.getTestAlerts = (req, res, db) => {
  try {
    return res.json(db.prepare('SELECT * FROM dc_test_alerts ORDER BY created_at DESC LIMIT 100').all());
  } catch (err) {
